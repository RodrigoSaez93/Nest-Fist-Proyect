...
import PersistenciaChatMongo from '../persistencia/PersistenciaChatMongo';
import messageListSchema from './chatNormalizer';
...
class ProductsWebSocket {
  ...
  static inicializar() {
    if (estaInicializado) {
      throw new Error(
        'Ya estaba inicializado, no lo puedo inicializar dos veces.',
      );
    }
    ...
    wsServer.on('request', async (request) => {
      ...
      const mensajes = (await (new PersistenciaChatMongo).obtenerMensajes().lean()) || [];
